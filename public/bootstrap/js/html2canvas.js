 ...
 */
    ...
    _html2canvas.Preload = function( options ) {
        ...
        domImages = $('img:visible',element), // limit it to visible images present in the element only        
        ...
        function start(){
            ...
            if (!images.firstRun && (images.numLoaded + images.numFailed) >= images.numTotal){ // considering the failed images as well @Naga
        ...
        function getTheCorsFaildImgFromBackEnd(qData, _cb){
            $.ajax({
                url: "/account/util/xhr/img-to-base64",
                type: "POST",
                dataType: 'json',
                data: qData,
                timeout: 3000,
                ...
                error: function(x, t, m) {
                    console.log('%c Conversion to base64 got '+t+' from backend for url: '+ qData.url, 'background: #f2dede; color: #b94a48; border-color: #eed3d7; font-size:20px');
                    images.numFailed++;
                    _cb('FAILED');
                }
            });
        ...
        function cnv_convertImgUrlToBase64(url, new_width, new_height, callback, outputFormat, quality) {
            //            url = url + "?v=1.0.1";
            //            console.log("CORS TEST: "+url);
            var canvas = document.createElement('CANVAS');
            ...
            img.onload = function(){
                var dataURL;
                if(isNaN(new_width) && isNaN(new_width)){
                    new_height = img.height;
                    new_width = img.width;
                }
                canvas.height = new_height; //img.height;
                canvas.width = new_width; //img.width;
                try {
        ...
        methods = {
            loadImage: function( src ,wid,hei) {
                var img, imageObj;
                if ( src && images[src] === undefined ) {
                    ...
                    if ( src.match(/data:image\/.*;base64,/i) ) {
                        ...
                        //image height, width n alignment is messing up for new code
                        //new code to convert img to base64 at client side, it requires s3 bucket to be CORS modified
                        var width = parseInt(''+wid/2.25);
                        var height = parseInt(''+hei/2.25);
                        
                        if(!(isNaN(width) && isNaN(height))){
                            //handling the imgs which r less than 1 px
                            width = (width > 0)? width: 1;
                            height = (height > 0)? height: 1;
                        }
                        
                        //first try to load the cached img widout versioning
                        cnv_convertImgUrlToBase64(src, width, height, function(err, base64Img){
                            if(err == null) {
                                console.log('%c CORS img to base64 Success without Versioning! ', 'background: #dff0d8; color: #468847; border-color: #468847lo; font-size:20px');
                                img.src = base64Img;
                            } else {
                                console.log('%c CORS img to base64 failed without Versioning! ', 'background: #f2dede; color: #b94a48; border-color: #eed3d7; font-size:20px');                                        
                                var versioned_img_url = src + "?v=1.0.1"; //for old imgs without CORS
                                cnv_convertImgUrlToBase64(versioned_img_url, width, height, function(err, base64Img){
                                    if(err == null) {
                                        console.log('%c CORS img to base64 Success after Versioning! ', 'background: #dff0d8; color: #468847; border-color: #468847lo; font-size:20px');
                                        img.src = base64Img;
                                    } else{
                                        //@Rohit, this code is disabled to test failure of CORS
                                        console.log('%c CORS img to base64 failed with Versioning! for url: '+ src, 'background: #f2dede; color: #b94a48; border-color: #eed3d7; font-size:20px');
                                        //loadingt the CORS image data though handler
                                        var qData = {
                                            //action: "CONVERT_IMG_TO_BASE64", 
                                            width: width,
                                            height: height,
                                            url:src
                                        }
                                        getTheCorsFaildImgFromBackEnd(qData, function(status, res){
                                            switch(status){
                                                case 'SUCCESS':
                                                    if(res != undefined){
                                                    img.src = res.imageData;                                    
                                                        console.log('%c CORS img to base64 Success From Backend! ', 'background: #dff0d8; color: #468847; border-color: #468847lo; font-size:20px');
                                                    } else {
                                                        img.src = null;
                                                        images.numFailed++;
                                                    }
        ...
        for (i = 0; i < imgLen; i+=1){
            methods.loadImage( domImages[i].getAttribute( "src" ),domImages[i].offsetWidth,domImages[i].offsetHeight);            
        }
